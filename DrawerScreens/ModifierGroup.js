...
export default function Basic({navigation}) {
  ...
  useEffect(() => {                   
    const unsubscribe = navigation.addListener('focus', () => {
      //use modifier_group/lists
      fetch('http://testweb.izaap.in/moop/api/index.php/service/modifiers/list?X-API-KEY=MoopApp2021@!',{
        method: 'GET'
        //Request Type 
        })
...
//use modifier_group/remove
    fetch(`http://testweb.izaap.in/moop/api/index.php/service/modifiers/removes?X-API-KEY=MoopApp2021@!&modifier_id=${data.item.id}`, {
      method: 'GET',
...
const searchFilterFunction = (text) => {
  ...
  if (text) {
    ...
    const newData = listData.filter(
      function (item) {
        const itemData = item.modifier_group_name
          ? item.modifier_group_name.toUpperCase()
          : ''.toUpperCase();
